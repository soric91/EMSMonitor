...
/**
 * Arma el informe mensual y lo descarga.
 *
...
const NOMBRE_MES = new Intl.DateTimeFormat('en-CA', {
  timeZone: 'America/Bogota',
  year: 'numeric',
  month: '2-digit',
});

function mesActual(): { mes: string; desde: string; hasta: string } {
  const ahora = new Date();
  const [anio, mesNumero] = NOMBRE_MES.format(ahora).split('-');
  const desde = new Date(`${anio}-${mesNumero}-01T05:00:00.000Z`);
  return { mes: `${anio}-${mesNumero}`, desde: desde.toISOString(), hasta: ahora.toISOString() };
}
...
export function MonthlyReportButton() {
  ...
  const generar = async () => {
    ...
    setGenerando(true);
    try {
      const { mes, desde, hasta } = mesActual();
      const rango = { from: desde, to: hasta, device_id: selectedDeviceId };
  ...
  return (
    ...
    >
      ...
      {generando ? 'Generando informe…' : 'Informe del mes (PDF)'}
    </button>
  );
}
